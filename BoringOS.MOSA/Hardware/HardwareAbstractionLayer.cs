// Taken from https://github.com/mosa/MOSA-Project/blob/master/Source/Mosa.Demo.CoolWorld.x86/HAL/Hardware.cs
...
public class HardwareAbstractionLayer : BaseHardwareAbstraction
{
	...
	{
		uint start = (uint)address.ToInt32();

		// Map physical memory space to virtual memory space
		for (uint at = start; at < start + size; at += PageSize)
	...
	{
		Pointer address = KernelMemory.AllocateVirtualMemory(size);
